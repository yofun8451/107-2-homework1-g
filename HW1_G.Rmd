...
要有可修個經的資格才可以看到個經的PR，只有實驗組沒有對照組。
虛擬變數的功能是為了區分實驗組跟對照組，當我們設定*可修個經*為虛擬變數時，可以抓到大一沒過經原的人，但當他有*個經PR*時（在大一之後修個經），他仍然需要通過經原以及修過微積分，所以雖然當他*可修個經*=0 時，他的*個經PR*仍然是在有符合經原及格和修過微積分且不為0 的兩者條件（大一後），故無法好好分辨所謂實驗組和對照組，而且當他*可修個經*=0時，無法確定是因為他「經原不及格」還是「微積分的條件沒過」，如果他為前者且之後他重修「經原」之後及格再去修個經，可能會導致個經成績更好，畢竟修兩次經原對個經成績有幫助，不然應該不會以「經原及格與否」來當作擋修條件。
...
假設$E(\epsilon_i|x_i)=0$，證明上題設定下：
  ...
  $v(\hat{\beta}_1|X)=v((\hat{\beta}_1-{\beta})|X)=v(\sum({x_i}-\bar{x})\epsilon_i)/\sum{({x_i-\bar{x}})^2}|x)=E[(\sum({x_i}-\bar{x})\epsilon_i)/\sum{({x_i-\bar{x}})^2})^2|X]-[E(\sum({x_i}-\bar{x})\epsilon_i)/\sum{({x_i-\bar{x}})^2}|X)]^2$
$[E(\sum({x_i}-\bar{x})\epsilon_i)/\sum{({x_i-\bar{x}})^2}|X)]^2=0 because E(\epsilon_i|X_i)=0$
$ so =1/[\sum{({x_i-\bar{x}})^2})^2\sum{E[({x_i-\bar{x}})^2\epsilon_i^2|x]} = \sigma_i^2/\sum{({x_i-\bar{x}})^2} = \sigma_i^2/(n_0n_1/n_0+n_1) = n\sigma_i^2/n_0n_1 $
